...
public class DashboardService {
        ...
        public List<MonthlyBreakdown> getMonthlyBreakdowns(int year) {
                ...
                for (MonthlyTypeSummary row : summaryRaw) {
                        ...
                        if (row.getType() == RecordType.INCOME)
                                t[0] += row.getTotal();
                        else
                                t[1] += row.getTotal();
                }
                ...
                return totalPivot.entrySet().stream()
                                .map(e -> {
                                        ...
                                        List<MonthlyCategorySummary> monthRows = byMonth.getOrDefault(m, List.of());
        ...
        public List<CategoryTrendPoint> getCategoryTrend(Long categoryId, CategoryType type) {
                List<CategoryTrendProjection> raw = (type == CategoryType.BOTH)
                                ? recordRepository.findCategoryTrend(categoryId)
                                : recordRepository.findCategoryTrend(categoryId, RecordType.valueOf(type.name()));
        ...
        public PagedResponse<RecordResponse> getRecentTransactions(int page, int size) {
                Pageable pageable = PageRequest.of(page, size,
                                Sort.by(Sort.Direction.DESC, "transactionDate", "id"));

                Page<FinancialRecord> recordPage = recordRepository.findAll(pageable);
                List<FinancialRecord> records = recordPage.getContent();
        ...
        // GET /dashboard/last5
        // =========================================================================
        public List<RecordResponse> getLast5() {
